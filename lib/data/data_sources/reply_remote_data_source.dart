...
abstract class ReplyRemoteDataSource {
  ...
  Future<bool> editComment(String? commentId, String? commentText);
}
  ...
  Future<bool> deleteComment(String? commentId) async {
    ...
    try {
      Response? response = await _apiClient.deleteReq(
        url: "${ApiConstants.deleteComment}/$commentId",
        jwtToken: jwtToken,
      );
      ...
      if (response?.data != null && response?.statusCode != null) {
        if (response!.statusCode == 200) {
          ...
          if (responseMsg == "Comment deleted successfully") {
            return true;
          } else {
            throw AppError(errorMessage: responseMsg);
          }
        } else {
          throw AppError(statusCode: response.statusCode);
        }
      ...
      log("deleteComment Error", error: e);
      throw AppError(errorMessage: e.toString());
    }
  }

  @override
  Future<bool> editComment(String? commentId, String? commentText) async {
    var jwtToken = await getJwtToken();
    var body = {
      'comment': commentText,
      'isEdited': true,
    ...
    try {
      Response? response = await _apiClient.patchReq(
        url: "${ApiConstants.editComment}/$commentId",
        body: body,
        jwtToken: jwtToken,
      );
